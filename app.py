...
@st.cache_resource
def build_retriever(confidence_threshold: float) -> FAQRetriever:
...
def render_results(response: dict[str, Any]) -> None:
    ...
    right.metric("Categoria", str(top_result["categoria"]))
...
def main() -> None:
    ...
    with st.form("search-form"):
        ...
        top_k = col_top_k.slider("Top-k", min_value=1, max_value=5, value=DEFAULT_TOP_K)
    ...
    retriever = build_retriever(confidence_threshold)
